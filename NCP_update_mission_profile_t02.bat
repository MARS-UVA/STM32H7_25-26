...
if not "%BOARD_ID%" == "NUCLEO-H7S3L8" if not "%BOARD_ID%" == "NUCLEO-U575ZI-Q" if not "%BOARD_ID%" == "NUCLEO-H563ZI" if not "%BOARD_ID%" == "NUCLEO-N657X0-Q" if not "%BOARD_ID%" == "NUCLEO-H755ZI-Q" (
...
@REM if "%BOARD_ID%" == "NUCLEO-N657X0-Q" (
@REM     echo "Set the boot mode in development mode (BOOT1 switch position is 2-3, BOOT0 switch position doesn't matter)"
@REM     echo "Press any key to continue..."
@REM     pause >nul
@REM     STM32_Programmer_CLI.exe -c port=swd mode=ur -w %current_dir%%BOARD_ID%_Binaries\Bootloader.bin 0x70000000 --extload %CUBEPROGRAMMER%\ExternalLoader\MX25UM51245G_STM32N6570-NUCLEO.stldr
@REM     if %ERRORLEVEL% neq 0 goto :error
@REM     echo "Set the boot mode in boot from external Flash (BOOT0 switch position is 1-2 and BOOT1 switch position is 1-2)"
@REM     echo "Press the reset button then press any key to continue..."
@REM     pause >nul
@REM ) else (
@REM     STM32_Programmer_CLI.exe -c port=swd mode=ur -w %current_dir%%BOARD_ID%_Binaries\Bootloader.bin 0x08000000 --go
@REM     if %ERRORLEVEL% neq 0 goto :error
@REM )
...
@REM if "%BOARD_ID%" == "NUCLEO-N657X0-Q" (
@REM     echo "ST67W6X_CLI_LWIP not available for NUCLEO-N657X0-Q"
@REM     echo "Press any key to continue..."
@REM     pause >nul
@REM ) else if "%BOARD_ID%" == "NUCLEO-H7S3L8" (
@REM     STM32_Programmer_CLI.exe -c port=swd mode=ur -w %current_dir%%BOARD_ID%_Binaries\ST67W6X_CLI_LWIP_Boot.bin 0x08000000
@REM     if %ERRORLEVEL% neq 0 goto :error
@REM     STM32_Programmer_CLI.exe -c port=swd mode=ur -w %current_dir%%BOARD_ID%_Binaries\ST67W6X_CLI_LWIP_Appli.bin 0x70000000 --extload %CUBEPROGRAMMER%\ExternalLoader\MX25UW25645G_NUCLEO-H7S3L8.stldr
@REM     if %ERRORLEVEL% neq 0 goto :error
@REM ) else (
@REM     STM32_Programmer_CLI.exe -c port=swd mode=ur -w %current_dir%%BOARD_ID%_Binaries\ST67W6X_CLI_LWIP.bin 0x08000000 --go
@REM     if %ERRORLEVEL% neq 0 goto :error
@REM )
